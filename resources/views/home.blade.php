    ...
    <div class="container mx-auto px-4 md:px-6 relative z-10">
        <div class="max-w-2xl">
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                <a href="{{ route('menu') }}">
                    <button class="bg-[#F5A623] text-[#0F0F0F] px-6 py-3 rounded hover:bg-[#F5A623]/90 w-full sm:w-auto flex items-center gap-2">
                        Bekijk Menu
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            <div class="flex flex-col items-center">
                <div class="w-16 h-16 rounded-full bg-[#F5A623]/20 flex items-center justify-center mb-4">
                    <svg class="w-8 h-8 text-[#F5A623]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3m0 0v3m0-3h3m-3 0H9"></path></svg>
                </div>
            ...
            <div class="flex flex-col items-center">
                <div class="w-16 h-16 rounded-full bg-[#F5A623]/20 flex items-center justify-center mb-4">
                    <svg class="w-8 h-8 text-[#F5A623]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 2l3 6 6 .5-4.5 4 1 6-5-3-5 3 1-6-4.5-4L9 8 12 2z"></path></svg>
                </div>
            ...
            <div class="flex flex-col items-center">
                <div class="w-16 h-16 rounded-full bg-[#F5A623]/20 flex items-center justify-center mb-4">
                    <svg class="w-8 h-8 text-[#F5A623]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3"></path></svg>
                </div>
...
<section class="py-20">
    <div class="container mx-auto px-4 md:px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-[#1a1a1a] border-white/10 overflow-hidden rounded-lg shadow group hover:border-[#F5A623]/50 transition-all">
                <div class="aspect-[4/3] overflow-hidden">
                    <img src="{{ $dish->image_url }}" alt="{{ $dish->name }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                </div>
                <div class="p-6">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <form action="{{ route('cart.add', $dish->id) }}" method="POST">
                            @csrf
                            <button class="bg-[#F5A623] text-[#0F0F0F] px-4 py-2 rounded hover:bg-[#F5A623]/90">Toevoegen</button>
                        </form>
                    </div>
                </div>
            </div>
        ...
        <div class="text-center mt-12">
            <a href="{{ route('menu') }}">
                <button class="border-[#F5A623] text-[#F5A623] px-6 py-3 rounded hover:bg-[#F5A623]/10">Bekijk Volledige Menu</button>
            </a>
        </div>
    </div>
</section>
...
<section class="py-20 bg-[#1a1a1a]">
    <div class="container mx-auto px-4 md:px-6 grid lg:grid-cols-2 gap-12 items-center">
        <div>
            ...
            <a href="{{ route('over-ons') }}" class="border border-[#F5A623] text-[#F5A623] px-6 py-3 rounded hover:bg-[#F5A623]/10">Lees Meer Over Ons</a>
        </div>
        <div class="grid grid-cols-2 gap-4">
            <img src="https://source.unsplash.com/400x400/?sushi,fresh" alt="Fresh Sushi" class="w-full aspect-square object-cover rounded-lg">
            <img src="https://source.unsplash.com/400x400/?japanese,food" alt="Japanese Food" class="w-full aspect-square object-cover rounded-lg">
            <img src="https://images.unsplash.com/photo-1562560471-cb5b5f96c1ab?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=1080" alt="Interior" class="w-full aspect-square object-cover rounded-lg">
            <img src="https://images.unsplash.com/photo-1730324772289-b00b3cfbd374?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&w=1080" alt="Chef" class="w-full aspect-square object-cover rounded-lg">
        </div>
    </div>
</section>
...
<section class="py-20">
    <div class="container mx-auto px-4 md:px-6 text-center">
        <h2 class="text-3xl md:text-4xl font-bold text-white mb-4">Wat Onze Gasten Zeggen</h2>
        <p class="text-white/60 mb-12">Ontdek waarom onze gasten steeds terugkomen</p>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach($reviews as $review)
            <div class="bg-[#1a1a1a] border-white/10 p-6 rounded-lg shadow">
                <div class="flex gap-1 mb-4">
                    @for($i=0; $i < $review['rating']; $i++)
                        <svg class="h-5 w-5 text-[#F5A623] fill-current" viewBox="0 0 24 24">
                            <path d="M12 .587l3.668 7.568L24 9.423l-6 5.853L19.336 24 12 20.201 4.664 24 6 15.276 0 9.423l8.332-1.268L12 .587z"/>
                        </svg>
                    @endfor
                </div>
                <p class="text-white/80 mb-4 leading-relaxed">"{{ $review['text'] }}"</p>
                <p class="text-[#F5A623] font-semibold">{{ $review['name'] }}</p>
            </div>
            @endforeach
        </div>
    </div>
